/*
    Linux driver for Tundra universeII PCI to VME bridge, kernel 2.4.x
...
static const char Version[] = "0.94 (February 2006)";

//#define VMIC
#ifdef VMIC
    #include "vmic.h"
    static void *VmicBaseAddr;
...
static ssize_t universeII_read(struct file *, char *, size_t, loff_t *);
static ssize_t universeII_write(struct file *, const char *, size_t,
                                loff_t *);
...
static void *baseaddr = 0;             // Base addres of Tundra chip

static void *dmaBuf = 0;               // DMA buf addr in kernel space
...
static void irq_handler(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    if (!status)             // we use shared ints, so we first check
        return;              // if this irq origins from universeII chip

    statistics.irqs++;
    if (status & 0x00FE)     // VMEbus interrupt
    {
    ...
    if (status & 0x0100)     // DMA interrupt
        wake_up_interruptible(&dmaWait);

    if (status & 0xF0000)    // mailbox interrupt
        for (i = 0; i < 4; i++)
            if (status & (0x10000 << i))
                wake_up_interruptible(&mbxWait[i]);

    if (status & 0x1000)     // IACK interrupt
        wake_up_interruptible(&vmeWait);

    if (status & 0x0400) {   // VMEBus error
...
static void execDMA(u32 chain)
{
    ...
    del_timer(&DMA_timer);
    remove_wait_queue(&dmaWait, &wait);
}
...
//
//----------------------------------------------------------------------------
static ssize_t universeII_read(struct file *file, char *buf,
...
//
//----------------------------------------------------------------------------
static ssize_t universeII_write(struct file *file, const char *buf,
                                size_t count, loff_t *ppos)
{
    int i = 0, okcount = 0, offset = 0, berr = 0;
    unsigned int dw, pci = 0;
...
static int universeII_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    {
        if (vma->vm_end - vma->vm_start > p->phys_end - p->phys_start) {
        ...
        retval = remap_page_range(vma->vm_start, p->phys_start,
        ...
        if (minor == DMA_MINOR) // DMA
        {
            ...
            retval = remap_page_range(vma->vm_start, dmaHandle,
                               vma->vm_end - vma->vm_start, vma->vm_page_prot);
            ...
            retval = remap_page_range(vma->vm_start, p->buffer,
                               vma->vm_end - vma->vm_start, vma->vm_page_prot);
        }
    }
                            ...
                            unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case IOCTL_SET_IMAGE: {
                ...
                for (i = 0; i < MAX_IMAGE; i++)
                    if ((image[i].opened == 2) && (i != minor)) {
                        ...
                        if (! ((iRegs.base <= imageStart)
                            || (iRegs.base + iRegs.size >= imageEnd))) {
                            ...
                            printk("imageStart1 = %x, imageEnd1 = %x, "
                                   ...
                                   imageStart, imageEnd);
                            return -4;  // overlap with existing image
                        }
                    }
        ...
        case IOCTL_GEN_VME_IRQ: {
                ...
                remove_wait_queue(&vmeWait, &wait);
        ...
        case IOCTL_WAIT_IRQ: {
                ...
                init_waitqueue_head(&irq_device[vmeIrq][vmeStatus].irqWait);
                add_wait_queue(&irq_device[vmeIrq][vmeStatus].irqWait, &wait);
                ...
                remove_wait_queue(&irq_device[vmeIrq][vmeStatus].irqWait, &wait);
                if (irqData.timeout > 0) {
                    del_timer(vTimer);
        ...
        case IOCTL_WAIT_MBX: {
                ...
                add_wait_queue(&mbxWait[mbxNr], &wait);
                current->state = TASK_INTERRUPTIBLE;
                ...
                remove_wait_queue(&mbxWait[mbxNr], &wait);
        ...
        case IOCTL_ADD_DCP: {
                ...
                if (ptr == NULL)
                    ...
                    ptr->dcp.dcpp = virt_to_bus(&(newP->dcp.dctl));
        ...
        case IOCTL_EXEC_DCP: {
                ...
                writel(virt_to_phys(cpLists[arg].commandPacket), baseaddr+DCPP);
...
static int __init universeII_init(void)
{
    ...
    universeII_dev = pci_find_device(PCI_VENDOR_ID_TUNDRA,
                              PCI_DEVICE_ID_TUNDRA_CA91C042, universeII_dev);
    if (!universeII_dev) {
    ...
    baseaddr = (void *) ioremap_nocache(ba, 4096);
    if (!baseaddr) {
    ...
    irq &= 0x000000FF;
    result = request_irq(irq, irq_handler, SA_INTERRUPT | SA_SHIRQ,
